...
def naive_softmax_loss_and_gradient(
    ...
    words_count = outsideVectors.shape[1]
    y = np.zeros(words_count)
    y[o] = 1
    y = y.reshape(-1, 1)

    y_hat = softmax(vc.T @ U).T
    loss = -np.log(y_hat[o][0])
...
def neg_sampling_loss_and_gradient(
    ...
    loss = -np.log(sigmoid(uo @ vc))
    for k in negSampleWordIndices:
        uk = U[:, k]
        loss -= np.log(sigmoid(-uk @ vc))

    dvc = 0.
    for k in negSampleWordIndices:
        uk = U[:, k]
        dvc += uk * (1 - sigmoid(-(uk @ vc)))
    dvc += -(1 - sigmoid(uo @ vc)) * uo

    dU = np.zeros_like(U)
    dU[:, o] += -vc * (1 - sigmoid(uo @ vc))
    for k in negSampleWordIndices:
        up = U[:, k]
        dU[:, k] += vc * (1 - sigmoid(-up @ vc))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    """
    V = centerWordVectors.T
    U = outsideVectors.T
    centerIdx = word2Ind[currentCenterWord]
    vc = V[:, centerIdx]

    loss = 0.0
    gradCenterVecs = np.zeros(V.shape)
    gradOutsideVectors = np.zeros(U.shape)

    dV = gradCenterVecs
    dU = gradOutsideVectors
    loss = 0.


    for ow in outsideWords:
        dloss, ddvc, ddU = word2vecLossAndGradient(vc, word2Ind[ow], U, dataset)

        loss += dloss
        dU += ddU
        dV[:, centerIdx] += ddvc
    return loss, dV.T, dU.T
